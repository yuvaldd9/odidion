...
class service:
    ...
    @staticmethod
    def bind_and_set_service(ip, port, bufsiz, service_name):
        try:
            service.UDP_IP = ip
            service.UDP_PORT = port
            service.BUFSIZ = bufsiz
            service.SERVICE_NAME = service_name
            service.PUBLIC_KEY, service.PRIVATE_KEY  =  onion_encryption_decryption.generate_keys((os.getcwd()), SERVICE_NAME)
    ...
    def send_to_client(id_key, data_to_send):
        send_back_onion_packet.generate_packet(id_key, service.CLIENTS_KEYS[id_key],\
                        service.BACK_ID_DICT[id_key]['ip'], service.BACK_ID_DICT[id_key]['port'], data_to_send)
